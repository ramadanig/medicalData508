        ...
        return new Promise<NgbModalRef>((resolve, reject) => {
            ...
            if (id) {
                this.doctorService.find(id).subscribe((doctor) => {
                    if (doctor.birthdate) {
                        doctor.birthdate = {
                            year: doctor.birthdate.getFullYear(),
                            month: doctor.birthdate.getMonth() + 1,
                            day: doctor.birthdate.getDate()
                        };
                    }
                    if (doctor.startDate) {
                        doctor.startDate = {
                            year: doctor.startDate.getFullYear(),
                            month: doctor.startDate.getMonth() + 1,
                            day: doctor.startDate.getDate()
                        };
                    }
